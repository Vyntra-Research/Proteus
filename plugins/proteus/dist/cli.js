        ...
        case "run":
            {
                ...
                const run = (0, chimera_1.runChimeraSession)(db, id, getNumber(parsed, "timeout"), { internalRun: getBoolean(parsed, "internal-run") });
...
function cmdCampaign(db, subcommand, parsed) {
    requireInitialized(db);
...
function cmdList(db, subcommand, parsed) {
    ...
    if (subcommand === "campaigns") {
        ...
        if (rows.length === 0)
            console.log("No campaigns recorded.");
        return;
    }
    ...
    if (subcommand === "links") {
        ...
        for (const row of rows) {
            ...
            if (row.note)
                console.log(`  ${truncateForCli(row.note, 180)}`);
        }
        if (rows.length === 0)
...
function requiredRoundStatus(parsed, key) {
...
function campaignStatus(parsed) {
    const status = getString(parsed, "status");
    if (status === undefined)
        return undefined;
