...
const tools: ToolDefinition[] = [
  ...
  },
  {
    ...
    handler: (input) =>
      withDb(str(input.root), (db) =>
        toolEnvelope(startChimeraSession(db, {
          role: str(input.role),
          goal: str(input.goal),
  ...
  },
  {
    ...
    inputSchema: schema(
      { root: stringProp("Target root path."), id: stringProp("Chimera session id."), timeout: numberProp("Timeout seconds.") },
      ["root", "id"]
    ),
    handler: (input) => withDb(str(input.root), (db) => toolEnvelope(runChimeraSession(db, str(input.id), maybeNum(input.timeout))))
  },
  {
...
function chimeraAccess(value: unknown): ChimeraAccessMode {
  if (value === undefined || value === null || value === "") return "lab";
