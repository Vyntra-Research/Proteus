...
import { ProteusDb, createDefaultContract, type ChimeraSessionRow, type CoverageRow, type RecordStatusReview } from "./db";
    ...
    case "snapshot": {
      ...
      if (body !== undefined) {
        console.log(JSON.stringify({
          ok: true,
          mode: "write",
          message: snapshotChimeraSession(db, currentChimeraSessionId(db, parsed, "id"), body)
        }, null, 2));
        return;
      }
      console.log(JSON.stringify({
...
function cmdCampaign(db: ProteusDb, subcommand: string | undefined, parsed: ParsedArgs): void {
  ...
  if (subcommand === "checkpoint") {
    ...
    if (contractSignature === undefined) throw new Error("--contract-signature must be valid JSON.");
...
function cmdQuery(db: ProteusDb, subcommand: string | undefined, parsed: ParsedArgs): void {
  ...
  if (subcommand === "duplicates") {
    ...
    for (const row of rows) {
      ...
      if (review) printLifecycleReviewGuidance(review);
    }
    return;
  }
...
function printPossibleDuplicateGuidance(db: ProteusDb, rows: CoverageRow[]): void {
...
function printLifecycleReviewGuidance(review: RecordStatusReview, prefix = "  "): void {
  ...
  if (command) console.log(`${prefix}review decisions, then reconcile explicitly if needed: ${command}`);
}
